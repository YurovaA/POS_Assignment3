...
int adjust_i_A(int, int);
int adjust_i_B(int, int);
void coords(int i, int size, int* x, int* y);
int coords_to_i(int x, int y, int sq_size);
...
int main (int argc, char **argv) {
	...
	double *A_local_block = NULL, *A_local_block_new = NULL, *B_local_block = NULL, *B_local_block_new = NULL, *C_local_block = NULL;
	...
	MPI_Request recv_request_2, send_request_1, send_request_2;
	...
	MPI_Cart_create(MPI_COMM_WORLD, 2, dimensions, periods, 0, &cartesian_grid_communicator);
		...
		if ((fp = fopen (argv[1], "r")) != NULL){
			...
			for (row = 0; row < matrices_a_b_dimensions[0]; row++){
				...
				for (column = 0; column < matrices_a_b_dimensions[1]; column++) {
					...
					A[row][column] += row;// row * column - 15 * row;
				}
			}
			fclose(fp);
		} else {
		...
		if((fp = fopen (argv[2], "r")) != NULL){
			...
			for(row = 0; row < matrices_a_b_dimensions[2]; row++){
				...
				for(column = 0; column < matrices_a_b_dimensions[3]; column++) {
					...
					B[row][column] -= column; // row * column + 3 * row - column;
				}
			}
			fclose(fp);
		} else {
	...
	A_local_block_new = (double *) malloc (A_local_block_size * sizeof(double));
	...
	B_local_block_new = (double *) malloc (B_local_block_size * sizeof(double));
	...
	if(rank == 0) {
		int i;
		for(i = 1; i < size; i++){
			//printf("A: %d \t %d \n", i, adjust_i_A(i, sqrt(size)));
			MPI_Send((A_array + (i * A_local_block_size)), A_local_block_size, MPI_DOUBLE, adjust_i_A(i, sqrt(size)), 1, cartesian_grid_communicator);
			//printf("B: %d \t %d \n", i, adjust_i_B(i, sqrt(size)));
			MPI_Send((B_array + (i * B_local_block_size)), B_local_block_size, MPI_DOUBLE, adjust_i_B(i, sqrt(size)), 0, cartesian_grid_communicator);
		}
		...
		MPI_Recv(A_local_block, A_local_block_size, MPI_DOUBLE, 0, 1, cartesian_grid_communicator, &status);
	...
	int flag1, flag2;
	double compute_time = 0, mpi_time = 0, start_comm, start_comp;
	...
	for(cannon_block_cycle = 0; cannon_block_cycle < sqrt_size; cannon_block_cycle++){
		...
		MPI_Isend(A_local_block, A_local_block_size, MPI_DOUBLE, (coordinates[1] + sqrt_size - 1) % sqrt_size, 0, row_communicator, &send_request_1);
		// rotate blocks vertically
		MPI_Isend(B_local_block, B_local_block_size, MPI_DOUBLE, (coordinates[0] + sqrt_size - 1) % sqrt_size, 0, column_communicator, &send_request_2);
		double wt = MPI_Wtime() - start_comm;
		rcv_snd += wt;
		mpi_time += wt;
		...
		for(C_index = 0, A_row = 0; A_row < A_local_block_rows; A_row++){
			...
			if (C_index % 2 == 0) {
				if (!flag1) {
					MPI_Test(&recv_request_1, &flag1, &status);
				} 
			
				if (!flag2) {
					MPI_Test(&recv_request_2, &flag2, &status);
				}
			}
			wt = MPI_Wtime() - start_comm;
			test_time += wt;
			mpi_time += wt;
			start_comp = MPI_Wtime();
	...
	double overall_rcv_snd;
	double overall_test_time;
	...
	MPI_Reduce(&rcv_snd, &overall_rcv_snd, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	MPI_Reduce(&test_time, &overall_test_time, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	MPI_Reduce(&wait_time, &overall_wait_time, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	...
	if (rank == 0) {
		...
		printf("Receive Send time:         %lf\n", overall_rcv_snd/size);
		printf("Test time:         %lf\n", overall_test_time/size);
		printf("Wait time:         %lf\n", overall_wait_time/size);
		//printf("(%d,%d)x(%d,%d)=(%d,%d)\n", A_rows, A_columns, B_rows, B_columns, A_rows, B_columns);
		//printf("Computation time: %lf\n", compute_time);
		//printf("MPI time:         %lf\n", mpi_time);
...
int adjust_i_A(int i, int sq_size) {
	int x, y;
	coords(i, sq_size, &x, &y);
	int shift_col = y - x < 0 ? (sq_size + y - x) % sq_size : (y - x) % sq_size;
	return coords_to_i(x, shift_col, sq_size);
}

int adjust_i_B(int i, int sq_size) {
	int x, y;
	coords(i, sq_size, &x, &y);
	int shift_row = x - y < 0 ? (sq_size + x - y) % sq_size : (x - y) % sq_size;
	return coords_to_i(shift_row, y, sq_size);
}

void coords(int i, int sq_size, int* x, int* y) {
	*y = i % sq_size;
	*x = i / sq_size;
}
